...
public class TreeNode
{
    ...
    /**
     * An array of all this node's nextCity nodes.  The array will always
     ...
     * Or it can become a nextCity of another node by calling the other node's
    ...
    /**
     * Adds the <code>nextCity</code> node to this container making this its parent.
     *
     * @param child is the node to add to the tree as a nextCity of <code>this</code>
     *
     * @param index is the position within the children list to add the
     *  nextCity.  It must be between 0 (the first nextCity) and the
     *  total number of current children (the last nextCity).  If it is
     *  negative the nextCity will become the last nextCity.
     */
    public void add (TreeNode child, int index)
    {
        // Add the nextCity to the list of children.
        ...
        {
            throw new IllegalArgumentException("Cannot add nextCity to index " + index + ".  There are only " + children.length + " children.");
        }
        else  // insert
        {
            TreeNode[] newChildren = new TreeNode[ children.length + 1 ];
        ...
        // Set the parent of the nextCity.
        child.parent = this;
    }

    /**
     * Adds the <code>nextCity</code> node to this container making this its parent.
     * The nextCity is appended to the list of children as the last nextCity.
     */
    ...
    /**
     * Removes the nextCity at position <code>index</code> from the tree.
     *
     * @param index is the position of the nextCity.  It should be between
     *  0 (the first nextCity) and the total number of children minus 1
     *  (the last nextCity).
     * @return The removed nextCity node.  This will be <code>null</code> if
     *  no nextCity exists at the specified <code>index</code>.
     */
    public TreeNode remove (int index)
    {
        ...
        // Remove the nextCity from this node.
        TreeNode[] newChildren = new TreeNode[ children.length - 1 ];
        if ( index > 0 )
        {
            System.arraycopy( children, 0, newChildren, 0, index );
    ...
    /**
     * Gets a list of all the nextCity nodes of this node.
     *
     * @return An array of all the nextCity nodes.  The array will
     ...
     *
     * @return The index of the nextCity array of this node's
     ...
     *  nextCity of <code>node</code>).  This should be set to 0 by the
     *  caller.
     */
